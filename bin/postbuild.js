...
if (!fs.existsSync(path.join(dirname, baseDir, staticDir))){
  fs.mkdirSync(path.join(dirname, baseDir, staticDir));
}

// same for the assetsDir directory
if (!fs.existsSync(path.join(dirname, baseDir, assetsDir))){
  fs.mkdirSync(path.join(dirname, baseDir, assetsDir));
}
...
fs.readdir(`./${baseDir}`, (err, files) => {
  ...
  assets.forEach(
    name => {
      fs.rename(path.join(dirname, baseDir, name), path.join(dirname, baseDir, assetsDir,  name), function (err) {
        if (err) throw err
  ...
  staticAssets.forEach(
    name => {
      fs.rename(path.join(dirname, baseDir, name), path.join(dirname, baseDir, staticDir,  name), function (err) {
        if (err) throw err
        console.log(`Successfully moved ${name}`)
      })
    }
